...
- [x] Spec requirement still not fully guaranteed for `git pull` failure output: `openspec/changes/add-rtk-git-compression/specs/agent-exec-run/spec.md:37-43` requires failure output to preserve error-bearing lines, but `src/compress.rs:545-553` checks `parse_shortstat(...)` and up-to-date success text before delegating to `summarize_git_push`, where the error-preservation branch lives at `src/compress.rs:514-517`. A `git pull` output containing both an `error:`/`fatal:` line and a shortstat-looking line would be summarized as `ok {files} files +{insertions} -{deletions}`, dropping the error-bearing line. Moved the `contains_git_error`/`error_bearing_git_lines` check to the start of `summarize_git_pull` and added a unit assertion covering pull failure with a shortstat-looking line (verification: unit - `src/compress.rs::tests::git_push_pull_branch_and_stash_summarize`, run by `cargo test git_`; job `7a5caaaa56e0dfc5511ed85b846ddafa` exit code 0).
