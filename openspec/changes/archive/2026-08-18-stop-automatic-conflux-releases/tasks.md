...
- The `agent-exec-distribution` spec delta lives in `specs/agent-exec-distribution/spec.md` within this change. Canonical `openspec/specs/` promotion is owned by the archive step, so apply does not edit the canonical spec directly.
- evidence: `cargo test --test integration conflux_on_merged_hook_has_no_release_side_effects` passed (1 passed; 0 failed).
- evidence: `cargo fmt -- --check` and `cargo clippy --tests --all-features -- -D warnings` both clean.
