...
#### Scenario: updated notify command is used at completion time

Given `agent-exec run -- echo hi` creates job `<job_id>`
And `agent-exec notify set <job_id> --command 'cat >/tmp/event.json'` is executed before `<job_id>` reaches a terminal state
When the job finishes
Then the completion notification command executed for `<job_id>` is `cat >/tmp/event.json`
