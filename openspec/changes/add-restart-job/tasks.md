...
- [ ] Add the `restart` CLI surface in `src/main.rs` and route it from the command dispatcher.
...
- [ ] Implement restart execution using the persisted job definition from `meta.json` without generating a new job id.
...
- [ ] Terminate running jobs before relaunching them.
...
- [ ] Treat `created` jobs as start-equivalent restart targets.
...
- [ ] Reset per-run artifacts before the fresh supervisor launch.
...
- [ ] Preserve persisted metadata and masking semantics during restart.
...
- [ ] Return `RunData`-compatible restart responses with inline observation controls.
...
- [ ] Preserve auto-GC and error behavior expectations.
...
- [ ] Update user-facing documentation and command schema surfaces that enumerate subcommands.
