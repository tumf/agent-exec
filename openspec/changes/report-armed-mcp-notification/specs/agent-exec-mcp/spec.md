...
MCP `run` の成功結果は CLI `run` と同じ `type="run"` response envelope を含まなければならない（MUST）。Completion sink を永続化した job が non-terminal state で返る場合、結果は optional structured `notification` object を含み、`state="armed"`、`polling_required=false`、delivery classification、および completion 時に通知されるため repeated `wait`/`status`/`tail` polling が不要であることを示す message を返さなければならない（MUST）。Sink が永続化されていない場合、または admission が失敗した場合、結果は notification が armed であると示してはならない（MUST NOT）。`armed` は terminal dispatch 用sinkが永続化されたことを意味し、downstream delivery成功を保証してはならない（MUST NOT）。
