...
- [x] 実コミット経路の archive gate が指摘した verification note を、過去の job ID 参照から repository-verifiable な証拠へ修正すること。実コミット経路の検証定義は `prek.toml` にあり、`prek run -a` は exit 0 で完了した。
